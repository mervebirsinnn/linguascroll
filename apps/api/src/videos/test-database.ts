 ...
 * transcript_segment_learning_points, Chunk 16 ile word_review_events —
 * hepsi tek bir TRUNCATE ifadesinde listeleniyor (Postgres, aynı ifadede
 * listelenen FK'li tabloları CASCADE'e gerek kalmadan birlikte temizler).
 */
export async function truncateTestTables(db: NodePgDatabase): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE videos, quizzes, quiz_options, users, video_watch_events, quiz_answer_events, words, video_words, user_saved_words, video_transcript_segments, transcript_segment_learning_points, word_review_events`,
  );
}
